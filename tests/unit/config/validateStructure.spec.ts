...
const emptyContentMatchMonitor = { ...sampleMonitor, type: MonitorType.content_match, pattern: '' };
const malformedRegexContentMatchMonitor = { ...sampleMonitor, type: MonitorType.content_match, pattern: '/(/' };
...
const nonBoolSecureParam = { ...sampleEmailNotifier, secure: 'false' };
...
describe('config/validateStructure', () => {
  ...
  it('Catches empty [monitor.*.pattern] (type=content_match)', () => expect(validate(withMonitor(emptyContentMatchMonitor))).toGiveError('EMPTY', ['monitors', 'test', 'pattern']))
  it('Passes non-regex [monitor.*.pattern] (type=content_match)', () => expect(validate(withMonitor(basicContentMatchMonitor))).toPass())
  it('Catches malformed regex [monitor.*.pattern] (type=content_match)', () => expect(validate(withMonitor(malformedRegexContentMatchMonitor))).toGiveError('MALFORMED_REGEX', ['monitors', 'test', 'pattern']))
  ...
  it('Catches non-boolean [email_notifier.*.secure]', () => expect(validate(withEmailNotifier(nonBoolSecureParam))).toGiveError('NOT_BOOLEAN', ['email_notifier', 'secure']))
